...
module.exports.faq = function(application, req, res){
      ...
      faqService.getAllFaq(false, 'categories=20', function (error, response, body) {
          if (!error && response.statusCode == 200){
              ...
              console.log("[APP Legislativo] Success ! FaqService return " + data.length + " results.");

              res.render('app-legislativo/faq/index', { result: data, error: {} });
          }else{
              res.render('app-legislativo/faq/index', { result: {}, error: error });
          };
      });
};
         ...
         if (!error && response.statusCode == 200){
            ...
            AEIService.getImpugnacoes('categories=19', function (error, response, body) {
                    if (!error && response.statusCode == 200){
                        ...
                        res.render('app-legislativo/aei/index', { avisos: avisos, impugnacoes: impugnacoes, error: {} });

                    }else{
                        //console.log(error);
                        res.render('app-legislativo/aei/index', { avisos: {},  impugnacoes: {}, error: error });
                    }
            });
